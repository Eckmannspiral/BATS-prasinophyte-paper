...
micromonas_only <- micromonas_only %>% filter (Species %in% c( "Micromonas_Clade-B_lagoon-and-innerseas(100)", "Micromonas_Clade-B_lagoon-and-innerseas(98)")) %>% 
  add_count(V4_ID, wt = copies, name = "total_cand2_sample") 

micromonas_only2 <- micromonas_only[!duplicated(micromonas_only), ]

micromonas_only2 = select(micromonas_only2, -Species) 

micromonas_only2$percent_cand2_micromonas <- (micromonas_only2$total_cand2_sample/micromonas_only2$total_micromonas_sample)
...
v1v2_micromonas_only <- v1v2_micromonas_only %>% filter (name %in% c( "seq1156")) %>%  add_count(Sample, wt = copies, name = "total_seq1156_sample") 

v1v2_micromonas_only$percent_seq1156_micromonas <- (v1v2_micromonas_only$total_seq1156_sample/v1v2_micromonas_only$total_micromonas_sample_v1v2)
...
v1v2_v4_micromonas_metadata = select(v1v2_v4_micromonas_metadata, `gsub("BS227_S65", "BS_227_S65", Sample)`, total_micromonas_sample_v1v2, total_seq1156_sample, percent_seq1156_micromonas, V4_18s_ID, V4_ID, total_micromonas_sample, total_cand2_sample, percent_cand2_micromonas) 
...
#choose just samples with seq1156 and/or cand2

v1v2_v4_micromonas_metadata <- v1v2_v4_micromonas_metadata[v1v2_v4_micromonas_metadata$total_seq1156_sample != 0 | v1v2_v4_micromonas_metadata$total_cand2_sample !=0,]


shapiro.test(v1v2_v4_micromonas_metadata$percent_seq1156_micromonas)

hist(v1v2_v4_micromonas_metadata$percent_seq1156_micromonas)

shapiro.test(v1v2_v4_micromonas_metadata$percent_cand2_micromonas)

hist(v1v2_v4_micromonas_metadata$percent_cand2_micromonas)

cor.test(v1v2_v4_micromonas_metadata$percent_seq1156_micromonas, v1v2_v4_micromonas_metadata$percent_cand2_micromonas, method="spearman", exact = FALSE)

#now samples with at least 10 total Micromonas amplicons

v1v2_v4_micromonas_metadata.ten <- subset(v1v2_v4_micromonas_metadata, total_micromonas_sample >= 10 & total_micromonas_sample_v1v2 >= 10)

cor.test(v1v2_v4_micromonas_metadata.ten$percent_seq1156_micromonas, v1v2_v4_micromonas_metadata.ten$percent_cand2_micromonas, method="spearman", exact = FALSE)

#linear regression

lm_all = lm(percent_seq1156_micromonas~percent_cand2_micromonas, data = v1v2_v4_micromonas_metadata)

summary(lm_all)

bptest(lm_all)

lm_10 = lm(percent_seq1156_micromonas~percent_cand2_micromonas, data = v1v2_v4_micromonas_metadata.ten)
...
ggplot(v1v2_v4_micromonas_metadata, aes(percent_cand2_micromonas, percent_seq1156_micromonas)) +geom_point(size=2, shape=23) + theme_classic()

all_graph_lm <- v1v2_v4_micromonas_metadata %>%
  ggplot(aes(x=percent_cand2_micromonas,y=percent_seq1156_micromonas)) +
  geom_point(alpha=1) +
  geom_smooth(method=lm, se=FALSE) + theme_classic() + xlim(0,1) + ylim(0,1)

all_graph_lm

ggplot(v1v2_v4_micromonas_metadata.ten, aes(percent_cand2_micromonas, percent_seq1156_micromonas)) +geom_point(size=2, shape=23) + xlim(0,1) + ylim(0,1) + theme_classic()

ten_graph_lm <- v1v2_v4_micromonas_metadata.ten %>%
  ggplot(aes(x=percent_cand2_micromonas,y=percent_seq1156_micromonas)) +
  geom_point(alpha=1) +
  geom_smooth(method=lm, se=FALSE) + theme_classic() + xlim(0,1) + ylim(0,1)
